...
function somdn_woo_download( $file_path, $product_id ) {
...
function somdn_download_multi_checked() {
	...
	//echo '<br>checkbox total = ' . $checked_total;
	//echo '<br>all checked = ' . $all_checked;
	...
	if ( $checked_total <= 0 ) {
		//$error = new WP_Error( 'broke', __( "I've fallen and can't get up", "my_textdomain" ) );
...
function somdn_zip_all_download_files( $downloads, $title ) {
	...
	//$downloadID = get_current_user_id() . $now->format("YdmGisu");
	$downloadID = get_current_user_id() . $now->format("uGYidms");
	...
	//if true, good and filepath returned; if false, zip creation failed
